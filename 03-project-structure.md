...
- **前端**：React + TypeScript（新） + 传统 HTML/JS（旧）
- **AI 能力**：集成多种 LLM API，支持 S2.5 和 S3 智能体
- **Live2D**：2D 角色展示
- **Steam**：Steam Workshop 集成
- **Docker**：容器化部署支持

> 💡 **提示**：关于环境搭建、项目克隆和依赖安装，请参考 [环境搭建指南](./01-getting-started.md)。关于 Git 和 GitHub 操作，请参考 [Git 与 GitHub 完整指南](./02-git-github-guide.md)。
...
- **作用**：提供 Web 界面和 API 接口
- **框架**：FastAPI
- **端口**：默认 48911（详见 [环境搭建指南 - 启动服务器](./01-getting-started.md#6-启动服务器)）
- **功能**：
  - 提供前端页面
  - 处理 API 请求
  - 管理角色配置
...
- **作用**：管理 AI 的记忆存储和检索
- **端口**：默认 48912（详见 [环境搭建指南 - 启动服务器](./01-getting-started.md#6-启动服务器)）
- **功能**：
  - 存储对话记忆
  - 生成记忆摘要
  - 语义搜索记忆
...
> 💡 **提示**：关于如何启动服务器，请参考 [环境搭建指南 - 启动服务器](./01-getting-started.md#6-启动服务器)。
...
- **planner.py**：规划任务步骤
- **processor.py**：执行任务
- **computer_use.py**：控制计算机（键鼠操作）
- **mcp_client.py**：MCP 协议通信
- **deduper.py**：去重处理
- **task_executor.py**：任务执行器
- **s2_5/**：S2.5 版本智能体实现
  - 支持基础智能体功能
...
- **core.py**：对话核心流程
- **omni_realtime_client.py**：实时流式对话
- **omni_offline_client.py**：非实时对话
- **tts_helper.py**：文本转语音
...
- **api_config_loader.py**：API 配置加载
- **audio.py**：音频处理
- **config_manager.py**：配置管理
- **frontend_utils.py**：前端工具函数
- **logger_config.py**：日志配置
- **preferences.py**：用户偏好设置
- **web_scraper.py**：网页爬虫
- **workshop_utils.py**：Steam Workshop 工具

### 8. Steamworks 模块（Steam 集成）

位于 `steamworks/` 目录，提供 Steam API 集成：

- **interfaces/**：Steam API 接口封装
  - **apps.py**：应用接口
  - **friends.py**：好友接口
  - **workshop.py**：创意工坊接口
  - 等等
- **enums.py**：Steam 枚举定义
- **methods.py**：Steam 方法定义
- **structs.py**：Steam 结构体定义
...
> 💡 **提示**：关于如何使用 `uv` 管理依赖和 `pyproject.toml`，请参考 [环境搭建指南 - 配置 Python 项目环境](./01-getting-started.md#4-配置-python-项目环境)。
...
- **env.template**：环境变量模板
- **config/core_config.json.example**：配置示例文件

### 打包配置

位于 `specs/` 目录：

- **launcher.spec**：启动器打包配置（PyInstaller）
- **monitor_build.spec**：监控服务打包配置
...
- [环境搭建指南](./01-getting-started.md) - 了解如何搭建开发环境和启动项目
...
- [开发实战入门](./04-development-basics.md) - 完成你的第一个功能
- [前端开发教程](./05-frontend-development.md) - 前端开发详解
- [后端开发教程](./06-backend-development.md) - 后端开发详解
